...
private:
  ...
  std::vector<unsigned char> currUpperBound;
  std::vector<unsigned char> currLowerBound;
  ...
  int convertPath2Number(std::deque<unsigned char> inPath, int upto=0);
  bool checkRange(std::vector<unsigned char> inPath);
  bool checkPathPrefix(std::vector<unsigned char> inPath);
...
public:
  ...
  virtual void setErrorPair(std::pair <std::string, unsigned int> inerrPair) {
	...
	virtual void enableShikharDebug() {
		shikharDebug = false;
	}
